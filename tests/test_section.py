...
# Make an idealized test data set
...
data_vars = {
    ...
    "dz": (Z + YX, z_w[1:, :, :] - z_w[:-1, :, :]),
    "u": (("s_rho", "eta_rho", "xi_u"), np.multiply.outer(np.ones(kmax), xx[:, :-1])),
    "v": (("s_rho", "eta_v", "xi_rho"), np.multiply.outer(np.ones(kmax), yy[:-1, :])),
...
grid = xgcm.Grid(ds, coords=grid_coords)
...
def test_idealized():
    ...
    assert np.allclose(dss.xi_c, [1.1, 1.4, 2.2])
    assert np.allclose(dss.eta_c, [1.0, 1.4, 2.1])
